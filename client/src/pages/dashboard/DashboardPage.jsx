...
} from "../../api/agreementApi"; // New import
    ...
    {agreements.length > 0 ? (
      <ul className="space-y-4">
        {agreements.map((agreement) => (
          <li
            key={agreement._id}
            className="bg-surface p-4 rounded-lg shadow-md"
          >
            <div className="flex flex-col sm:flex-row justify-between sm:items-center">
              <div>
                ...
                <div className="text-sm text-textSecondary mt-2">
                  ...
                  <ul className="list-disc list-inside ml-4">
                    {agreement.learningObjectives.map((obj, i) => (
                      <li key={i}>{obj}</li>
                    ))}
                  </ul>
                  <p className="mt-1">
                    Duration: {agreement.proposedDuration} minutes
                  </p>
                </div>
              </div>
              <div className="flex space-x-2 mt-4 sm:mt-0">
                <Button
                  variant="outline"
                  onClick={() => onStatusUpdate(agreement._id, "declined")}
                >
                  Decline
                </Button>
                <Button
                  onClick={() => onStatusUpdate(agreement._id, "accepted")}
                >
                  Accept
                </Button>
              </div>
    ...
    {exchanges.length > 0 ? (
      <ul className="space-y-4">
        {exchanges.map((exchange) => (
          <li
            key={exchange._id}
            className="bg-surface p-4 rounded-lg shadow-md flex justify-between items-center"
          >
            <div>
              <p className="font-semibold">
                {exchange.initiator.fullName} &harr;{" "}
                {exchange.receiver.fullName}
              </p>
              <p className="text-sm text-textSecondary">
                Learning "{exchange.topicToLearn.name}" &amp; Teaching "
                {exchange.topicToTeach.name}"
              </p>
            </div>
...
const DashboardPage = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const exchangeData = Array.isArray(exchangesResponse.data)
        ? exchangesResponse.data
        : [];
      setExchanges(exchangeData);
      const agreementsData = Array.isArray(agreementsResponse.data)
      ...
      const newStats = {
        ...
        pending: agreementsData.filter(
          (a) => a.status === "pending" && a.receiver._id === user._id,
        ).length,
        completed: exchangeData.filter((e) => e.status === "completed").length,
      };
      setStats(newStats);

      const registeredEvents = Array.isArray(registeredEventsRes.data)
        ? registeredEventsRes.data
        : [];
      const hostedEvents = Array.isArray(hostedEventsRes.data)
        ? hostedEventsRes.data
        : [];
      const combinedEvents = [
        ...registeredEvents.map((e) => ({ ...e, type: "attending" })),
        ...hostedEvents.map((e) => ({ ...e, type: "hosting" })),
      ];
      combinedEvents.sort(
  ...
  const incomingAgreements = (
    Array.isArray(agreements) ? agreements : []
  ).filter((a) => a.status === "pending" && a.receiver._id === user?._id);
  const activeExchanges = (Array.isArray(exchanges) ? exchanges : []).filter(
    (e) => e.status === "accepted",
  );
  const completedExchanges = (Array.isArray(exchanges) ? exchanges : []).filter(
              ...
              >
                <div>
                  <p className="font-semibold">
                    {event.title}
                    <span
                      className={`text-xs font-semibold ml-2 px-2 py-0.5 rounded-full ${event.type === "hosting" ? "bg-yellow-200 text-yellow-800" : "bg-green-200 text-green-800"}`}
                    >
