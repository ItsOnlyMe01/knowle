...
const FindMatchesPage = () => {
  ...
  const getTopicPairForMatch = (match) => {
    const topicToLearnId = user.topicsToLearn.find((learnTopic) =>
      match.topicsToTeach.some(
        (teachTopic) => teachTopic._id === learnTopic._id,
      ),
    )?._id;

    const topicToTeachId = user.topicsToTeach.find((teachTopic) =>
      match.topicsToLearn.some(
        (learnTopic) => learnTopic._id === teachTopic._id,
      ),
    )?._id;
  ...
  if (loading)
    return (
      <p className="text-center">
        Asking the AI to find perfect matches for you... ✨
      </p>
    );
  ...
  return (
    <div>
      ...
      <div className="mb-6 max-w-xs mx-auto">
        ...
        >
          {t("findMatches.filterByLanguage")}
        </label>
        <select
          id="language-filter"
        ...
        >
          ...
          {(Array.isArray(spokenLanguages) ? spokenLanguages : []).map(
            (lang) => (
        ...
        <div className="text-center bg-red-100 text-red-700 p-4 rounded-md mb-6">
          <p>
            ...
            <Link to="/profile/edit" className="font-semibold underline">
              {t("findMatches.updateProfileLink")}
            </Link>
          </p>
        </div>
      )}
      {(Array.isArray(matches) ? matches : []).length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {(Array.isArray(matches) ? matches : []).map((match) => (
            <div
              key={match._id}
              className="bg-surface p-6 rounded-lg shadow-md flex flex-col transition-transform hover:scale-105"
            >
              ...
              <div className="bg-indigo-50 border-l-4 border-primary text-indigo-800 p-3 rounded-r-lg mb-4">
                <p className="text-sm font-semibold">
                  {t("findMatches.aiReason")}
                </p>
                <p className="text-sm">
                  "
                  {match.matchReason ||
                    "This user is a strong match for your learning goals!"}
                  "
                </p>
              </div>
              ...
              <div className="space-y-3 mb-4 flex-grow">
                <div>
                  <h4 className="font-semibold text-sm text-primary">
                    {t("findMatches.canTeach")}
                  </h4>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {(Array.isArray(match.topicsToTeach)
                      ? match.topicsToTeach
                      : []
                    ).map((t) => (
                      <span
                        key={t._id}
                        className="bg-indigo-100 text-primary text-xs font-semibold px-2.5 py-0.5 rounded-full"
                      >
                        {t.name}
                      </span>
                    ))}
                ...
                <div>
                  <h4 className="font-semibold text-sm text-secondary">
                    {t("findMatches.wantsToLearn")}
                  </h4>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {(Array.isArray(match.topicsToLearn)
                      ? match.topicsToLearn
                      : []
                    ).map((t) => (
                      <span
                        key={t._id}
                        className="bg-green-100 text-secondary text-xs font-semibold px-2.5 py-0.5 rounded-full"
                      >
                        {t.name}
                      </span>
                    ))}
