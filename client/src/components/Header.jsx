...
const Header = () => {
      ...
      [
        ...
        },
        {
          ...
          links: [
            {
              to: "/mentors",
              label: t("header.browseMentors"),
              condition: user,
            },
            {
              to: "/premium-content",
              label: t("header.premiumContent"),
              condition: user?.premium?.subscriptionStatus === "active",
            },
          ],
        },
      ].filter((item) => item.condition),
    [t, user, i18n.language],
  ); // Bug Fix: Add i18n.language to dependency array
  ...
  const renderNavLinks = (isMobile = false) => (
    <>
      ...
      <NavLink
        ...
        onClick={closeMenus}
        className={({ isActive }) =>
          `px-3 py-2 rounded-md text-sm font-medium ${isActive ? "text-primary bg-primary/10" : "text-textSecondary hover:text-primary hover:bg-primary/10"}`
        }
      >
        {t("header.home")}
      </NavLink>
      ...
      {(navStructure || []).map((item) => (
        <div key={item.category} className={isMobile ? "w-full" : "relative"}>
          ...
          >
            ...
            <svg
              className={`w-4 h-4 ml-1 transform transition-transform ${openDropdown === item.category ? "rotate-180" : ""}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div
            className={`transition ease-out duration-200 transform ${openDropdown === item.category ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"}`}
          >
            <ul
              className={
                isMobile
                  ? "pl-4 mt-2 space-y-2"
                  : "absolute right-0 mt-2 w-48 bg-surface rounded-md shadow-lg z-20 py-1"
              }
            >
              {Array.isArray(item.links)
                ? item.links.filter((link) => link.condition)
                : [].map((link) => (
                    <li key={link.to}>
                      <NavLink
                        to={link.to}
                        onClick={closeMenus}
                        className={({ isActive }) =>
                          `block w-full text-left px-4 py-2 text-sm ${isActive ? "text-primary font-bold" : "text-textSecondary hover:text-primary hover:bg-gray-100 dark:hover:bg-gray-700"}`
                        }
                      >
                        {link.label}
                      </NavLink>
  ...
  return (
    <>
      ...
      {/* Mobile Drawer */}
      <div
        className={`fixed inset-0 z-40 lg:hidden transform transition-transform duration-300 ease-in-out ${isDrawerOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div
        ...
        <div
          ...
          role="dialog"
          aria-modal="true"
          className="relative w-72 max-w-[80vw] h-full bg-surface shadow-xl flex flex-col p-4"
        >
          ...
          <nav className="flex-grow flex flex-col space-y-2 overflow-y-auto">
            ...
            {user ? (
              ...
              <div className="space-y-2 mt-4">
                <Link to="/login" onClick={closeMenus} className="w-full">
                  <Button variant="outline" className="w-full">
                    {t("header.login")}
                  </Button>
                </Link>
                <Link to="/register" onClick={closeMenus} className="w-full">
