...
const OnboardingStep = ({ question, onAnswer, currentAnswer }) => {
    ...
    return (
        <div>
            ...
            <div className={`grid bg-surface ${type === 'multi-select' ? 'grid-cols-2 md:grid-cols-3' : 'grid-cols-1 md:grid-cols-2'} gap-4`}>
                {options.map((option, index) => (
                    <button
                        key={index}
                        onClick={() => handleSelect(option)}
                        className={`block text-center  p-4 rounded-lg border-2 transition-all duration-200 transform hover:scale-105 ${isSelected(option)
