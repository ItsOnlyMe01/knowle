...
const userSchema = new Schema(
  {
    ...
    lastReviewSummaryReviewCount: {
      // For cost-effective AI caching
      type: Number,
      default: 0,
    },
    // MODIFICATION START
...
userSchema.methods.generateAccessToken = function () {
  // MODIFICATION START
  // The role must be included in the JWT payload so that other microservices
  // can perform authorization checks without needing to query the database.
  return jwt.sign(
    {
      _id: this._id,
      email: this.email,
...
// Indexes for optimizing queries, especially for matchmaking and filtering.
userSchema.index({ topicsToTeach: 1 });
userSchema.index({ topicsToLearn: 1 });
userSchema.index({ languagesSpoken: 1 });
userSchema.index({ averageRating: -1, numberOfRatings: -1 }); // Performance: For 'Top Rated' leaderboard
