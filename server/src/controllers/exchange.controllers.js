...
const findMatches = asyncHandler(async (req, res) => {
  ...
  if (
    currentUser.topicsToTeach.length === 0 ||
    currentUser.topicsToLearn.length === 0
  ) {
    return res
      .status(200)
      .json(
        new ApiResponse(
          200,
          [],
          "Please add topics you can teach and want to learn to find matches.",
        ),
      );
  }
  ...
  const potentialMatches = await User.find(matchQuery)
    ...
    .select(
      "fullName username avatar bio topicsToTeach topicsToLearn languagesSpoken",
    )
    .lean();
    ...
    return res
      ...
      .json(
        new ApiResponse(
          200,
          [],
          "No direct matches found with the current filters.",
        ),
      );
  }

  // try {
  //   const aiRankedMatches = await getAiPoweredMatches(
  //     currentUser,
  //     potentialMatches,
  //   );
  ...
  //     const indexA = aiRankedMatches.findIndex(
  //       (m) => m.id === a._id.toString(),
  //     );
  //     const indexB = aiRankedMatches.findIndex(
  //       (m) => m.id === b._id.toString(),
  //     );
  ...
  //         200,
  //         finalMatches,
  //         "AI-powered matches retrieved successfully",
  //       ),
  //     );
  // } catch (error) {
  ...
  //         200,
  //         potentialMatches,
  //         "Matches retrieved successfully (fallback)",
  //       ),
  //     );
  // }
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, potentialMatches, "Matches retrieved (without AI)"),
    );
});
...
const createExchange = asyncHandler(async (req, res) => {
  ...
  console.log("FREE LEFT BEFORE:", user.freeExchangesLeft);

  if (
    !user?.premium?.subscriptionStatus ||
    user.premium.subscriptionStatus !== "active"
  ) {
    if (user.freeExchangesLeft <= 0) {
      throw new ApiError(403, "Free limit reached. Upgrade to premium.");
    }
  }
  ...
  if (
    ...
    !mongoose.Types.ObjectId.isValid(topicToLearnId) ||
    !mongoose.Types.ObjectId.isValid(topicToTeachId)
  ) {
  ...
  if (existingExchange) {
    throw new ApiError(
      409,
      "An active or pending exchange request with this user already exists.",
    );
  }
  ...
  if (
    ...
    if (user.freeExchangesLeft > 0) {
      user.freeExchangesLeft -= 1;
      await user.save();
      console.log("FREE LEFT AFTER:", user.freeExchangesLeft);
    }
  }
  // --- Notification Logic ---
  ...
  return res
    .status(201)
    .json(
      new ApiResponse(201, exchange, "Exchange request sent successfully."),
    );
});
...
const getExchangeDetails = asyncHandler(async (req, res) => {
  ...
  const exchange = await Exchange.findById(exchangeId)
    .populate("initiator", "fullName username avatar slug")
    .populate("receiver", "fullName username avatar slug")
  ...
  if (
    exchange.initiator._id.toString() !== userId.toString() &&
    exchange.receiver._id.toString() !== userId.toString()
  ) {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, exchange, "Exchange details retrieved successfully"),
    );
});
...
const getUserExchanges = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, exchanges, "User exchanges retrieved successfully"),
    );
});
...
const updateExchangeStatus = asyncHandler(async (req, res) => {
  ...
  // MODIFICATION START
  // Removed 'completed' from this controller as it's now handled by confirmCompletion
  ...
  if (status === "cancelled") {
    ...
    if (exchange.status === "accepted" && !isInitiator && !isReceiver) {
      throw new ApiError(
        403,
        "Only participants can cancel an active exchange.",
      );
    }
  }
  ...
  const updatedExchange = await Exchange.findById(exchangeId).populate(
    "initiator receiver topicToLearn topicToTeach",
  );

  return res
    .status(200)
    .json(
      new ApiResponse(200, updatedExchange, `Exchange has been ${status}.`),
    );
});
...
const confirmCompletion = asyncHandler(async (req, res) => {
  ...
  if (exchange.status !== "accepted") {
    throw new ApiError(
      400,
      "Only an accepted exchange can be marked as completed.",
    );
  }
  ...
  if (isInitiator) {
    exchange.initiatorCompleted = true;
  } else {
    exchange.receiverCompleted = true;
  }

  // Check if both have confirmed
  if (exchange.initiatorCompleted && exchange.receiverCompleted) {
    exchange.status = "completed";

    // --- Execute all post-completion logic here ---

    // 1. Skill Node Completion Logic
    const initiatorLearnedTopicId = exchange.topicToLearn;
    const initiatorSkillNode = await SkillNode.findOne({
      topic: initiatorLearnedTopicId,
    });
    ...
    const receiverSkillNode = await SkillNode.findOne({
      topic: receiverLearnedTopicId,
    });
...
const submitReview = asyncHandler(async (req, res) => {
  ...
  if (exchange.status !== "completed")
    throw new ApiError(400, "Can only review completed exchanges.");
  ...
  if (isInitiator) {
    if (exchange.initiatorRating)
      throw new ApiError(400, "You have already reviewed this exchange.");
  ...
  // This check is intentionally placed here because skill verification depends on the quality
  // of the teaching, which is only known after a rating is submitted.
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, exchange, "Review submitted successfully."));
});
...
const uploadAudioRecording = asyncHandler(async (req, res) => {
  ...
  if (
    !exchange.initiator.equals(req.user._id) &&
    !exchange.receiver.equals(req.user._id)
  ) {
    throw new ApiError(403, "You are not a participant of this exchange.");
  }
  ...
  return res
    ...
    .json(
      new ApiResponse(
        201,
        newRecording,
        "Recording uploaded and transcription started.",
      ),
    );
});
...
const getRecordingsForExchange = asyncHandler(async (req, res) => {
  ...
  const recordings = await Recording.find({ exchange: exchangeId }).sort({
    createdAt: -1,
  });
  return res
    .status(200)
    .json(
      new ApiResponse(200, recordings, "Recordings retrieved successfully."),
    );
});
...
export {
  ...
  confirmCompletion, // MODIFICATION
  submitReview,
